...
import { getLeads, getStats, getPipelineStatus, updateLead, deleteLead, runPipeline } from '$lib/api';
...
export const load: PageServerLoad = async ({ url }) => {
  ...
  const [leadsData, stats, pipelineStatus] = await Promise.all([
    ...
    getStats().catch(() => ({ draft: 0, sent: 0, replied_won: 0, replied_lost: 0, archived: 0, total: 0 })),
    getPipelineStatus().catch(() => ({ last_run: null, leads_found: 0, errors: [], is_running: false }))
  ]);
