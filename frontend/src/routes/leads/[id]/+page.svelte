...
<div class="p-8 max-w-[900px] mx-auto space-y-8">

  <!-- Header -->
  <div class="flex items-start justify-between gap-4">
    <div>
      <a href="/" class="text-sm text-text-secondary hover:text-accent transition-colors inline-flex items-center gap-1 mb-3">
        ← Back to Leads
      </a>
      <div class="flex items-center gap-3">
        <h1 class="text-3xl font-bold text-text-primary">{data.lead.company_name}</h1>
  ...
  <div class="grid grid-cols-1 lg:grid-cols-[1fr_260px] gap-8">
    ...
    <div class="space-y-8">
      ...
      <!-- Company Information -->
      <section>
        <h2 class="text-base font-semibold text-text-primary mb-4 pb-2 border-b border-border">Company Information</h2>
        <dl class="grid grid-cols-2 gap-4 text-sm">
          <div>
            <dt class="text-text-secondary mb-1">Company</dt>
            <dd class="text-text-primary font-medium">{data.lead.company_name}</dd>
          </div>
          <div>
            <dt class="text-text-secondary mb-1">Region / Country</dt>
            <dd class="text-text-primary">{[data.lead.region, data.lead.country].filter(Boolean).join(', ') || '—'}</dd>
          </div>
          <div>
          ...
          {#if data.lead.company_description}
            <div class="col-span-2">
              ...
              <dd class="text-text-primary leading-relaxed">{data.lead.company_description}</dd>
            </div>
          {/if}
        </dl>
      </section>
      ...
      <!-- Funding Information -->
      <section>
        <h2 class="text-base font-semibold text-text-primary mb-4 pb-2 border-b border-border">Funding</h2>
        <dl class="grid grid-cols-2 gap-4 text-sm">
          <div>
            <dt class="text-text-secondary mb-1">Amount</dt>
            <dd class="text-text-primary font-medium">{data.lead.funding_amount ?? '—'}</dd>
          </div>
          <div>
          ...
          <div>
            ...
            <dd class="text-text-primary">{formatDate(data.lead.funding_date)}</dd>
          </div>
        </dl>
      </section>
      ...
      {#if data.lead.news_headline}
        <section>
          <h2 class="text-base font-semibold text-text-primary mb-4 pb-2 border-b border-border">Source</h2>
          <div class="text-sm">
            ...
            {:else}
              <p class="text-text-secondary leading-relaxed">{data.lead.news_headline}</p>
            {/if}
          </div>
        </section>
      {/if}
      ...
      <section>
        <h2 class="text-base font-semibold text-text-primary mb-4 pb-2 border-b border-border">Contact & Status</h2>
        <form
          method="POST"
          action="?/update-lead"
          class="space-y-4"
        ...
        >
          ...
          <div class="flex flex-col gap-1">
            <label for="status-select" class="text-sm font-medium text-text-secondary">Status</label>
            <select
              id="status-select"
              name="status"
              bind:value={status}
          ...
          <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <Input bind:value={cto_name} name="cto_name" label="CTO Name" placeholder="Jane Smith" />
            <Input bind:value={cto_email} name="cto_email" type="email" label="CTO Email" placeholder="jane@company.com" />
          </div>
          <Input bind:value={linkedin_url} name="linkedin_url" type="url" label="LinkedIn URL" placeholder="https://linkedin.com/in/..." />
          <TextArea bind:value={notes} name="notes" label="Notes" placeholder="Add your notes here..." rows={4} />
      ...
      <section>
        <div class="flex items-center justify-between mb-4 pb-2 border-b border-border">
          ...
          <div class="flex gap-2">
            <Button size="sm" variant="ghost" onclick={copyToClipboard} type="button">
              {copied ? '✓ Copied' : 'Copy'}
            </Button>
            <form
              method="POST"
            ...
            >
              <Button size="sm" variant="secondary" type="submit" loading={isRegenerating}>
                Regenerate
              </Button>
            </form>
          </div>
        ...
        {#if email_draft}
          <pre class="bg-bg-subtle border border-border rounded-md p-4 text-sm font-mono text-text-primary whitespace-pre-wrap leading-relaxed min-h-[200px] overflow-auto">{email_draft}</pre>
        {:else}
          <div class="bg-bg-subtle border border-border rounded-md p-8 text-center text-sm text-text-tertiary">
      ...
      <section class="pt-4 border-t border-border">
        <Button variant="danger" onclick={() => (showDeleteModal = true)} type="button">
          Archive Lead
        </Button>
      </section>
    ...
    <aside class="space-y-4">
      <div class="bg-bg-subtle border border-border rounded-md p-4 space-y-3 text-sm">
        <h3 class="font-semibold text-text-primary text-xs uppercase tracking-wider">Metadata</h3>
        <dl class="space-y-3">
          <div>
            <dt class="text-text-tertiary text-xs mb-0.5">Lead ID</dt>
            <dd class="font-mono text-text-secondary text-xs break-all">{data.lead.id}</dd>
          </div>
          <div>
            <dt class="text-text-tertiary text-xs mb-0.5">Created</dt>
            <dd class="text-text-primary">{formatDateTime(data.lead.created_at)}</dd>
          </div>
          <div>
            <dt class="text-text-tertiary text-xs mb-0.5">Updated</dt>
            <dd class="text-text-primary">{formatDateTime(data.lead.updated_at)}</dd>
          </div>
          {#if data.lead.sent_at}
            <div>
              <dt class="text-text-tertiary text-xs mb-0.5">Sent At</dt>
              <dd class="text-text-primary">{formatDateTime(data.lead.sent_at)}</dd>
            </div>
          {/if}
        </dl>
      </div>
...
<Modal isOpen={showDeleteModal} title="Archive this lead?" onClose={() => (showDeleteModal = false)}>
  <p class="text-sm text-text-secondary mb-6">
    This will permanently remove <strong class="text-text-primary">{data.lead.company_name}</strong> from your pipeline. This action cannot be undone.
  </p>
  <form method="POST" action="?/delete-lead" use:enhance>
    <div class="flex gap-2 justify-end">
      <Button variant="secondary" onclick={() => (showDeleteModal = false)} type="button">Cancel</Button>
